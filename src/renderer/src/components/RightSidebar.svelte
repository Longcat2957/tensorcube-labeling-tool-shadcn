<script lang="ts">
  ...
  function handleLabelClick(labelId: string, event: MouseEvent | KeyboardEvent) {
    if (event.shiftKey) {
      workspaceManager.toggleLabelSelection(labelId)
    } else {
            ...
            <div class="p-3">
              ...
              {#if workspaceManager.currentLabels.length > 0}
                <div class="space-y-1" role="list" aria-label="라벨 목록">
                  {#each workspaceManager.currentLabels as label (label.id)}
                    {@const isVisible = label.visible}
                    {@const isSelected = workspaceManager.selectedLabelIds.includes(label.id)}
                    <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
                    <!-- svelte-ignore a11y_no_noninteractive_tabindex -->
                    <div
                      class="w-full flex items-center gap-2 px-2 py-2 rounded-md text-sm transition-colors hover:bg-muted/50 border cursor-pointer {isSelected
                        ? 'border-primary bg-primary/10'
                        : 'border-transparent'}"
                      role="listitem"
                      onclick={(e) => handleLabelClick(label.id, e)}
                      onkeydown={(e) => e.key === 'Enter' && handleLabelClick(label.id, e)}
                      tabindex="0"
                    >
                      <span
                      ...
                      <div class="flex items-center gap-0.5 ml-auto">
                        <Button
                          ...
                          onclick={(e) => {
                            e.stopPropagation()
                            workspaceManager.toggleLabelVisibility(label.id)
                          }}
                        >
                          {#if isVisible}
