...
import { updateBoxPosition, updateBadgePosition, extractBBoxFromRect, extractOBBFromRect } from "../boxUtils.js";
...
export function addBoxToCanvas(
  ...
  const className = workspaceManager.workspaceConfig?.names?.[String(annotation.class_id)] ?? `Class ${annotation.class_id}`;
  ...
  rect.on('modified', () => {
    ...
    if (rect.data.shape === 'obb') {
      ...
      console.log('[Modified] OBB AFTER:', { cx: newObb[0], cy: newObb[1], w: newObb[2], h: newObb[3], angle: newObb[4] });
      ...
      console.log('[Modified] BB AFTER:', { xmin: newBbox[0], ymin: newBbox[1], xmax: newBbox[2], ymax: newBbox[3] });
    ...
    if (objects) {
      if (rect.data.shape === 'obb') {
        ...
        if (updated) {
          updateBadgePosition(objects.badge, updated.obb, scale, offset.x, offset.y, false, className);
        }
      } else {
        const updated = workspaceManager.getBBAnnotationById(annotation.id);
        if (updated) {
          updateBadgePosition(objects.badge, updated.bbox, scale, offset.x, offset.y, true, className);
        }
      }
    }
